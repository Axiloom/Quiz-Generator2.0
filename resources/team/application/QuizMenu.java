 ...
 *
 */
public class QuizMenu extends application.Main {
  ...
  private ArrayList<application.Question.QuestionNode> questions; // all questions in this current quiz
    ...
    next.setOnAction(e -> {
      ...
      if (activeBoxes.isEmpty()) {
        ...
        if (activeBoxes.get(0).getText().equals(questions.get(currentQuestion).answer)) {
          application.Main.getStatisticsMenu().correct++;
          application.Main.getStatisticsMenu().initialize(questions.size());
        }
        ...
        if (currentQuestion == questions.size() - 1) {
          currentQuestion = 0;
          primaryStage.setScene(application.Main.getStatisticsScene());
        } else {
          currentQuestion++;
          application.Main.getQuizMenu().initialize(questions);
        }
      }
    });
